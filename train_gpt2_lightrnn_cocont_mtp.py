...
from coconts import BigramTopK, topk_to_row_distribution
...
if __name__ == "__main__":
    ...
    # CoCoNT smoothing parameters
    parser.add_argument(
        "--use_cocont",
        action="store_true",
        help="Enable CoCoNT n-gram target smoothing",
    )
    parser.add_argument(
        "--cocont_alpha",
        type=float,
        default=0.1,
        help="Mixing weight for CoCoNT smoothing (0.0 disables)",
    )
    parser.add_argument(
        "--ngram_path",
        type=str,
        default="",
        help="Path to .npz bigram top-k model (required if --use_cocont)",
    )
    parser.add_argument(
        "--ngram_k",
        type=int,
    ...
    # ------------------------------------------------------------------
    if args.use_cocont:
        assert args.ngram_path, "--ngram_path must be provided when --use_cocont"
        ngram_model = BigramTopK(args.ngram_path, k=args.ngram_k).to(device)
        print0(f"Loaded BigramTopK model from {args.ngram_path} (K={args.ngram_k})")
    else:
        ngram_model = None
    ...
    for step in range(args.num_iterations + 1):
            ...
            with torch.no_grad():
                ...
                for _ in range(val_steps):  # always fiexed number of validation steps
                    ...
                    if args.use_cocont and raw_model.codebook is not None:
                        with torch.no_grad():
                            topk_tok_val, topk_prob_val = ngram_model.batch_lookup(x_val)
                            row_dist_val = topk_to_row_distribution(
                                topk_tok_val,
                                topk_prob_val,
                                raw_model.codebook.table_size,
                            )
                    else:
                        row_dist_val = None
                    # Ensure return_logits=False if LightRNN handles loss internally and its "logits" are not vocab-sized
        ...
        for micro_step in range(args.grad_accumulation_steps):
            ...
            with ctx:
                torch.compiler.cudagraph_mark_step_begin()
                if args.use_cocont and raw_model.codebook is not None:
                    # Build smoothed row distribution for current batch
                    with torch.no_grad():
                        topk_tok, topk_prob = ngram_model.batch_lookup(x)
                        row_dist = topk_to_row_distribution(
                            topk_tok,
                            topk_prob,
                            raw_model.codebook.table_size,
                        )
