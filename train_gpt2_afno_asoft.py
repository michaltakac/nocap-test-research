...
# Disable cudagraphs in TorchInductor to avoid runtime errors with index_put accumulations
os.environ.setdefault("TORCHINDUCTOR_DISABLE_CUDAGRAPHS", "1")
# Disable cudagraphs at torch.compile (Dynamo) level as well
os.environ.setdefault("TORCH_COMPILE_DISABLE_CUDAGRAPHS", "1")
import numpy as np
import torch
# Suggestion 3: enable efficient FFT plan cache for AFNO layers
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        b, t = idx.size()
        # pos = torch.arange(0, t, dtype=torch.long, device=idx.device)  # shape (t) # not used with RoPE or wpe
        ...
        if self.config.mixer == "afno":
            ...
            # For attention (RoPE), no explicit addition of wpe here
            x = tok_emb
...
if __name__ == "__main__":
    ...
    # AFNO specific arguments (copied from train_gpt2_rtx4090_optim_afno.py)
    ...
    parser.add_argument(
        "--compile",
        action="store_true",
        help="Enable torch.compile (Inductor). Disable by default due to stability issues with AFNO + NCE.",
    )
    ...
    model_config_obj = GPTConfig(**model_config_params)
    ...
    if args.compile:
        print0("compiling the model with torch.compile...")
        # Compile the rest of the model with reduce-overhead mode; cudagraphs already disabled via config.
        model = torch.compile(model, mode="reduce-overhead")
    else:
        print0("[INFO] --compile flag not set; running model in eager mode (safer).")

    # If adaptive softmax is active and compile is enabled, wrap asoft
    # The comment "wrap it so torch.compile skips it" or "to avoid expensive kernel autotune"
    # implies this is relevant when compilation is active.
    if use_asoft and hasattr(model, "asoft") and args.compile:
        print0("[INFO] Wrapping model.asoft with NonCompiledWrapper as --compile is enabled.")
        class NonCompiledWrapper(nn.Module):
            def __init__(self, module):
                super().__init__()
                self.module = module
                # Non-standard attribute removed: # self._is_compiling = True

            def forward(self, *args, **kwargs):
                return self.module(*args, **kwargs)

        # Replace asoft with non-compiled wrapper
    ...
    # Ensure cudagraphs are disabled globally **before** any torch.compile() call.
    if hasattr(config, "triton") and hasattr(config.triton, "cudagraphs"):
        config.triton.cudagraphs = False
    if hasattr(config, "cudagraphs"):
        config.cudagraphs = False

    # Ensure cudagraphs disabled through torch._dynamo as well (PyTorch <2.3 fallback)
    try:
        import torch._dynamo.config as dynamo_config
        if hasattr(dynamo_config, "cudagraphs"):  # older keys
            dynamo_config.cudagraphs = False
        if hasattr(dynamo_config, "use_cudagraphs"):
            dynamo_config.use_cudagraphs = False
    except ImportError:
        pass