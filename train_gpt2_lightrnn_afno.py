...
# Import AFNO mixer and Adaptive Softmax
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Select mixer: standard causal self-attention or AFNO spectral mixer
        if config.mixer == "afno":
            ...
            # AFNO keeps width unchanged; use same residual scaling factor
...
class GPTConfig:
    ...
    # Precision parameter
    precision: str = "bf16"
    # Mixer / architecture parameters
    bias: bool = False
    mixer: str = "attn"                 # "attn" or "afno"
...
class GPT(nn.Module):
    ...
    def __init__(self, config, *, use_asoft: bool = False, cutoffs=None, div_value: float = 4.0):
        super().__init__()
        self.config = config
        self.use_asoft = use_asoft and config.tie_embedding == "full"  # Adaptive Softmax only with full embedding

        # ------------------------------------------------------------------
        # Embedding & positional modules
        # ------------------------------------------------------------------
        modules = {}
        ...
        if config.tie_embedding == "light":
            ...
            modules["wte"] = LightRNNEmbedding(self.codebook, config.n_embd)
        else:
            modules["wte"] = nn.Embedding(config.vocab_size, config.n_embd)
            self.codebook = None

        # Positional embedding only needed if we use AFNO mixer
        if config.mixer == "afno":
            modules["wpe"] = nn.Embedding(config.sequence_length, config.n_embd)

        # Transformer blocks (either Attn or AFNO) --------------------------------
        ...
        if config.tie_embedding == "light":
            ...
            self.decoder = None
            if self.use_asoft:
                assert cutoffs is not None and len(cutoffs) > 0, "Cutoffs must be provided for adaptive softmax"
    ...
    # Forward pass
    # ------------------------------------------------------------------
    def forward(self, idx, targets=None, return_logits: bool = True):
        b, t = idx.size()

        # Token embeddings
        tok_emb = self.transformer.wte(idx)

        # Optional position embeddings for AFNO
        ...
        # Transformer / AFNO blocks
        for block in self.transformer.h:
            x = block(x)
        x = rmsnorm(x)

        # ------------------------------------------------------------------
        # Output & loss handling
        # ------------------------------------------------------------------
        if self.config.tie_embedding == "light":
            ...
            if self.use_asoft:
                ...
                if targets is not None:
                    loss_out = self.asoft(x_fp32.view(-1, x_fp32.size(-1)), targets.view(-1))
                    loss = loss_out.loss.to(x.dtype)
    ...
    # ------------------------------------------------------------------
    # Greedy generation helper (supports LightRNN and full vocab)
    # ------------------------------------------------------------------
    @torch.no_grad()
    def generate(self, idx, max_new_tokens: int = 32):
        ...
        for _ in range(max_new_tokens):
            ...
            if self.config.tie_embedding == "light":
                row_logits, col_logits = logits
                row_id = row_logits.squeeze(1).argmax(-1)
                col_id = col_logits.squeeze(1).argmax(-1)
                next_token = (row_id * self.codebook.table_size + col_id).clamp(max=self.config.vocab_size - 1).unsqueeze(1)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--adaptive_softmax",
        action="store_true",
        help="Use Adaptive Softmax instead of full softmax when tie_embedding is 'full'.",
    )
    parser.add_argument(
        "--asoft_cutoffs",
        type=str,
        default="2000,10000",
        help="Comma-separated cutoffs for adaptive softmax (e.g., '2000,10000').",
    )
    parser.add_argument(
        "--asoft_div_value",
        type=float,
    ...
    # AFNO mixer arguments
    parser.add_argument(
        "--mixer",
        type=str,
        default="attn",
        choices=["attn", "afno"],
        help="Token mixer: 'attn' for standard attention, 'afno' for spectral AFNO",
    )
    parser.add_argument("--afno_num_blocks", type=int, default=8, help="Number of AFNO blocks")
    parser.add_argument("--afno_sparsity_threshold", type=float, default=0.01, help="AFNO sparsity threshold")
    parser.add_argument("--afno_hard_thresholding_fraction", type=float, default=1.0, help="AFNO hard thresholding fraction")
    parser.add_argument("--afno_hidden_size_factor", type=int, default=1, help="AFNO hidden size expansion factor")
    parser.add_argument("--bias", action="store_true", help="Enable bias terms in Linear layers (mostly for experimentation)")
    ...
    # ---------------- Adaptive Softmax setup ----------------
    use_asoft = args.adaptive_softmax and args.tie_embedding == "full"
    asoft_cutoffs_parsed = []
    if use_asoft:
        if args.asoft_cutoffs.strip():
            asoft_cutoffs_parsed = [int(c.strip()) for c in args.asoft_cutoffs.split(',') if c.strip()]
        asoft_cutoffs_parsed = sorted({c for c in asoft_cutoffs_parsed if c < num_vocab})
        if not asoft_cutoffs_parsed:
            print0("Warning: adaptive_softmax requested but no valid cutoffs < vocab_size. Disabling.")
            use_asoft = False

    model = GPT(
        model_config,
        use_asoft=use_asoft,
        cutoffs=asoft_cutoffs_parsed,
        div_value=args.asoft_div_value,
    ).to(device, dtype=param_dtype)

    # Ensure adaptive softmax params remain in FP32 for numerical stability
    if use_asoft and hasattr(model, 'asoft') and model.asoft is not None:
