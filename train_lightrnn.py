...
class RNNLanguageModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.tie_embedding == "light":
            ...
            self.embedding = LightRNNEmbedding(self.codebook, config.n_embd)
        ...
        # Optional RMS norm on outputs
        self.norm = nn.LayerNorm(config.n_embd)

    # ------------------------------------------------------------
    def forward(self, idx, targets=None, return_logits=True):
        # idx: (B,T)
        emb = self.embedding(idx)  # (B,T,d)

        output, _ = self.rnn(emb)  # (B,T,d)
        output = self.norm(output)

        if self.config.tie_embedding == "light":
            logits, loss = self.decoder(output, targets)
        else:
    ...
    def generate(self, idx, max_new_tokens=32):
        ...
        for _ in range(max_new_tokens):
            logits, _ = self(idx[:, -1:], targets=None, return_logits=True)
            if self.config.tie_embedding == "full":
                ...
                token_id = token_id.clamp(max=self.config.vocab_size - 1)
                next_tok = token_id.unsqueeze(1)
            idx = torch.cat([idx, next_tok], dim=1)
        return idx

    # Optimizer helper (kept for training script compatibility)
...
if __name__ == "__main__":
    ...
    num_vocab = 50257
    model_config_params = {
        "d12": dict(n_layer=12, n_embd=768),
        "d24": dict(n_layer=24, n_embd=1024),
        "d36": dict(n_layer=36, n_embd=1280),
        "d48": dict(n_layer=48, n_embd=1600),
    }[args.model]
    ...
    model = RNNLanguageModel(model_config).to(device, dtype=param_dtype)
    model = model.train().cuda()
    if hasattr(config, "coordinate_descent_tuning"):
        config.coordinate_descent_tuning = True  # suggested by @Chillee
    print0("compiling the model...")
    # Disable Triton cudagraphs only if using full softmax path in Transformer due to buffer-reuse issues seen with Rotary caches.
    compile_opts = {"triton.cudagraphs": False}
    model = torch.compile(model, options=compile_opts)
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            with torch.no_grad():
                ...
                for _ in range(val_steps):  # always fiexed number of validation steps
                    ...
                    _, loss = model(x_val, y_val, return_logits= (raw_model.config.tie_embedding == "full"))
                    val_loss += loss
        ...
        for micro_step in range(args.grad_accumulation_steps):
            ...
            with ctx:
                ...
                _, loss = model(x, y, return_logits= (raw_model.config.tie_embedding == "full"))
